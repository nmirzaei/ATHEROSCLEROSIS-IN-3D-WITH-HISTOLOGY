...
def NC5_NonDim(kk,u,vtkfile2, vtkfile3, vtkfile4, vtkfile5,vtkfile6,volumefile1,volumefile2) :
        ...
        #Uploading mesh
        Domain=Mesh('eccentric_domain_3D_NC_small_bulged.xml')
        Bulk = MeshFunction('size_t' , Domain , 'eccentric_domain_3D_NC_small_bulged_physical_region.xml' )  #saves the interior info of the mesh
        Boundary = MeshFunction('size_t', Domain , 'eccentric_domain_3D_NC_small_bulged_facet_region.xml')
        ######################################################################
        ...
        l1 = Expression('1.1*exp(-s*(x[0]-c1)*(x[0]-c1)-s*(x[1]-c2)*(x[1]-c2))',degree=0, s=3,c1= 0.663,c2=0.712)
        l2 = Expression('1.1*exp(-s*(x[0]-c1)*(x[0]-c1)-s*(x[1]-c2)*(x[1]-c2))',degree=0, s=3,c1= 1,c2=-0.2)
        l3 = Expression('0.8*exp(-s*(x[0]-c1)*(x[0]-c1)-s*(x[1]-c2)*(x[1]-c2))',degree=0, s=3,c1= 0.04,c2=-1.11)
        l4 = Expression('0.8*exp(-s*(x[0]-c1)*(x[0]-c1)-s*(x[1]-c2)*(x[1]-c2))',degree=0, s=3,c1= 0.643,c2=-0.853)
        l5 = Expression('0.6*exp(-s*(x[0]-c1)*(x[0]-c1)-s*(x[1]-c2)*(x[1]-c2))',degree=0, s=3,c1= -0.67,c2=-0.95)
        LL = project(l1+l2+l3+l4+l5,W_1)
        #####################################################

        #Oxygen sources
        o1 = Expression('a*exp(-s*(x[0]-c1)*(x[0]-c1)-s*(x[1]-c2)*(x[1]-c2))',degree=0, s=40,c1= 0.33 ,c2= 0.64 ,a=200000-(200000*kk/6))
        o2 = Expression('a*exp(-s*(x[0]-c1)*(x[0]-c1)-s*(x[1]-c2)*(x[1]-c2))',degree=0, s=40,c1= 0.45 ,c2= 0.41,a=200000-(200000*kk/3))
        o3 = Expression('a*exp(-s*(x[0]-c1)*(x[0]-c1)-s*(x[1]-c2)*(x[1]-c2))',degree=0, s=40,c1= 0.34 ,c2= 0.94 ,a=200000-(200000*kk/9))
        o4 = Expression('a*exp(-s*(x[0]-c1)*(x[0]-c1)-s*(x[1]-c2)*(x[1]-c2))',degree=0, s=40,c1= 0.85 ,c2= 0.093 ,a=200000)
        o5 = Expression('a*exp(-s*(x[0]-c1)*(x[0]-c1)-s*(x[1]-c2)*(x[1]-c2))',degree=0, s=20,c1= 0.897 ,c2= -0.434 ,a=220000)
        o6 = Expression('a*exp(-s*(x[0]-c1)*(x[0]-c1)-s*(x[1]-c2)*(x[1]-c2))',degree=0, s=40,c1= 0.29 ,c2= -0.76 ,a=170000)
        o7 = Expression('a*exp(-s*(x[0]-c1)*(x[0]-c1)-s*(x[1]-c2)*(x[1]-c2))',degree=0, s=35,c1= 0.115 ,c2= -0.85 ,a=170000-(170000*kk/6))
        o8 = Expression('a*exp(-s*(x[0]-c1)*(x[0]-c1)-s*(x[1]-c2)*(x[1]-c2))',degree=0, s=35,c1= -0.241 ,c2= -0.901 ,a=170000)
        O1 = Expression('a*exp(-s*(x[0]-c1)*(x[0]-c1)-s*(x[1]-c2)*(x[1]-c2))',degree=0, s=40,c1= -0.112 ,c2= 0.742 ,a=140000*kk/3)
        O2 = Expression('a*exp(-s*(x[0]-c1)*(x[0]-c1)-s*(x[1]-c2)*(x[1]-c2))',degree=0, s=40,c1= 0.113 ,c2= 0.583 ,a=140000*kk/3)
        O3 = Expression('a*exp(-s*(x[0]-c1)*(x[0]-c1)-s*(x[1]-c2)*(x[1]-c2))',degree=0, s=60,c1= 0.25 ,c2= 0.349 ,a=130000*kk/3)
        O4 = Expression('a*exp(-s*(x[0]-c1)*(x[0]-c1)-s*(x[1]-c2)*(x[1]-c2))',degree=0, s=60,c1= 0.324 ,c2= 0.186 ,a=130000*kk/3)
        O5 = Expression('a*exp(-s*(x[0]-c1)*(x[0]-c1)-s*(x[1]-c2)*(x[1]-c2))',degree=0, s=30,c1= 0.896 ,c2= 0.475 ,a=300000*kk/3)
        O6 = Expression('a*exp(-s*(x[0]-c1)*(x[0]-c1)-s*(x[1]-c2)*(x[1]-c2))',degree=0, s=40,c1= 1.008 ,c2= 0.084 ,a=20000*kk/3)
        O7 = Expression('a*exp(-s*(x[0]-c1)*(x[0]-c1)-s*(x[1]-c2)*(x[1]-c2))',degree=0, s=40,c1= 0.815 ,c2= -0.757 ,a=170000*kk/3)
        O8 = Expression('a*exp(-s*(x[0]-c1)*(x[0]-c1)-s*(x[1]-c2)*(x[1]-c2))',degree=0, s=40,c1= 0.433 ,c2= -0.981 ,a=200000*kk/3)
        O9 = Expression('a*exp(-s*(x[0]-c1)*(x[0]-c1)-s*(x[1]-c2)*(x[1]-c2))',degree=0, s=40,c1= -0.583 ,c2= -0.831 ,a=200000*kk/3)
        O10 = Expression('a*exp(-s*(x[0]-c1)*(x[0]-c1)-s*(x[1]-c2)*(x[1]-c2))',degree=0, s=40,c1= 0.33 ,c2= 0.072 ,a=80000*kk/3)
        O11 = Expression('a*exp(-s*(x[0]-c1)*(x[0]-c1)-s*(x[1]-c2)*(x[1]-c2))',degree=0, s=60,c1= 0.316 ,c2= -0.19 ,a=80000*kk/3)
        O12 = Expression('a*exp(-s*(x[0]-c1)*(x[0]-c1)-s*(x[1]-c2)*(x[1]-c2))',degree=0, s=60,c1= 0.96 ,c2= -0.064 ,a=100000*kk/3)
        O13 = Expression('a*exp(-s*(x[0]-c1)*(x[0]-c1)-s*(x[1]-c2)*(x[1]-c2))',degree=0, s=60,c1= 0.52 ,c2= -0.63 ,a=100000*kk/3)

        OO = project(170000-(o1+o2+o3+o4+o5+o6+o7+o8+O1+O2+O3+O4+O5+O6+O7+O8+O9+O10+O11+O12+O13),W_1)
        #####################################################
        ...
        gamma_min = 0.03
        gamma_max = 12
        C_crit = 0.65 #This determines the lack of oxygen
        m = 4
        ######################################################################
        ...
        F= -nu_1*dot(grad(M),grad(v1))*dx+kappa_1*(1-M)*v1*ds(1)-kappa_2*M*v1*ds(2)+mu*M*dot(grad(L+Q),grad(v1))*dx-(gamma_min+(gamma_max-gamma_min)*(C_crit**m/(C_crit**m+C**m)))*M*v1*dx\
