...
def NC5_NonDim(kk,u,vtkfile2, vtkfile3, vtkfile4, vtkfile5,vtkfile6,volumefile1,volumefile2) :
        ...
        l = Expression('exp(-s*(x[0]+0.9-w1)*(x[0]+0.9-w1)-s*(x[1]+0.5+w2)*(x[1]+0.5+w2))',degree=0, s=0.7, w1 = 0.42*kk, w2= 0.2*kk)
        LL = project(l,W_1)
        #####################################################
        ...
        gamma_min = 0.03
        gamma_max = 12
        C_crit = 0.5  #This determines the lack of oxygen
        m = 4
        ######################################################################
        ...
        bcs = [DirichletBC(Mixed_Space.sub(0), 1, surface_marker, 1),DirichletBC(Mixed_Space.sub(2), 1, surface_marker, 1)]
        ######################################################################
