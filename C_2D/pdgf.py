...
def pdgf(counter,kk,u,vtkfile7,vtkfile8,a) :
        ...
        aa = Expression('sqrt(pow(x[0]-c,2))',degree=0,c=-0.23)
        bb = Expression('sqrt(pow(x[1]-c,2))',degree=0,c=-0.53)
        ...
        F= -nu*dot(grad(P),grad(v))*dx-beta*P*v*dx+kappa*conditional(lt(a,Constant(0.15)),1,0)*conditional(lt(b,Constant(0.15)),1,0)*(1-P)*v*ds(1)
        #######################################################################
