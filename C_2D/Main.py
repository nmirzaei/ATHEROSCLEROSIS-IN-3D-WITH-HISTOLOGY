...
#Upload the mesh
mesh = Mesh('eccentric_domain_3D_NC_small_bulged.xml')  #imports the converted mesh
###############################################################
...
# Facet functions
Volume = MeshFunction('size_t' , mesh , 'eccentric_domain_3D_NC_small_bulged_physical_region.xml' )  #saves the interior info of the mesh
bnd_mesh = MeshFunction('size_t', mesh , 'eccentric_domain_3D_NC_small_bulged_facet_region.xml')  #saves the boundary info of the mesh
###############################################################
...
while t<=t_max:
     ...
     GG = (T_i.T)*(G)*(T_i)
     ginv_i = inv(GG)
    # File('eccentric_domain_2D_NC_2_plots_main/Growth_Tensor.pvd')<<project(GG,TSpace)
     G_i = det(GG)
     if k>0:
