...
def pdgf(counter,kk,u,vtkfile7,vtkfile8,a) :
        ...
        Domain1 = Mesh('eccentric_domain_3D_NC_small_bulged.xml')
        Bulk1 = MeshFunction('size_t' , Domain1 , 'eccentric_domain_3D_NC_small_bulged_physical_region.xml' )  #saves the interior info of the mesh
        Boundary1 = MeshFunction('size_t', Domain1 , 'eccentric_domain_3D_NC_small_bulged_facet_region.xml')
        #######################################################################
        
        #Changing the directory to the lustre directory
        os.chdir("Lustre Drive")
        ######################################################################
        ...
        F= -nu*dot(grad(P),grad(v))*dx-beta*P*v*dx+kappa*conditional(lt(abs(a),Constant(0.15)),1,0)*conditional(lt(b,0),1,0)*(1-P)*v*ds(1)
        #######################################################################
