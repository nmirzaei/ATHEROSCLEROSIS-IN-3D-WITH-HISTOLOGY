...
def NC5_NonDim(kk,u,vtkfile2, vtkfile3, vtkfile4, vtkfile5,vtkfile6,volumefile1,volumefile2) :
        ...
        l = Expression('L_0*exp(-s*(x[0]-0.1)*(x[0]-0.1)-s*(x[1]+1.2)*(x[1]+1.2))',degree=0, s=0.14, L_0= 1-(kk)/300)
        LL = project(l,W_1)
        #####################################################
        ...
        gamma_min = 0.03
        gamma_max = 12
        C_crit = 0.5 # - kk/50#This determines the lack of oxygen
        m = 4
        ######################################################################
        ...
        bcs = [DirichletBC(Mixed_Space.sub(2), 1, surface_marker, 1),DirichletBC(Mixed_Space.sub(0), 1.3-kk/300, surface_marker, 1)]
        ######################################################################
