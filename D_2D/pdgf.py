...
def pdgf(counter,kk,u,vtkfile7,vtkfile8,cc) :
        ...
        aa1 = Expression('sqrt(pow(x[0]-c,2))',degree=0,c=-0.7)
        bb1 = Expression('sqrt(pow(x[1]-c,2))',degree=0,c=-0.37)
        aa2 = Expression('sqrt(pow(x[0]-c,2))',degree=0,c=0.0)
        bb2 = Expression('sqrt(pow(x[1]-c,2))',degree=0,c=0.7)
        aaa1 = project(aa1,W_1)
        bbb1 = project(bb1,W_1)
        aaa2 = project(aa2,W_1)
        bbb2 = project(bb2,W_1)
        #######################################################################
        ...
        a2 = interpolate(aaa2,SS)
        b2 = interpolate(bbb2,SS)
        #######################################################################
        ...
        F= -nu*dot(grad(P),grad(v))*dx-beta*P*v*dx+kappa*(conditional(lt(a1,Constant(0.25)),1,0)*conditional(lt(b1,Constant(0.25)),1,0)+conditional(lt(a2,Constant(0.35)),1,0)*conditional(lt(b2,Constant(0.35)),1,0))*(1-P)*v*ds(1)
        #######################################################################
